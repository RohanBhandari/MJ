...
{ 
  ...
  TH1F *h1_dR_Lep_Trk[7], *h1_nGoodIsoTrks[7];
  ...
  for(int i=0; i<7; i++) 
    {   
      ...
      h1_dRlep[i] = InitTH1F( Form("h1_%s_dRlep_%ifatjet", ch->GetTitle(), i), 
        ...
        h1_dR_Lep_Trk[i] = InitTH1F( Form("h1_%s_dR_Lep_Trk_%ifatjet", ch->GetTitle(), i), 
                                 ...
                                 20, 0, 4);	
        h1_nGoodIsoTrks[i] = InitTH1F( Form("h1_%s_nGoodIsoTrks_%ifatjet", ch->GetTitle(), i), 
                                 Form("h1_%s_nGoodIsoTrks_%ifatjet", ch->GetTitle(), i), 
                                 8, 0, 8);	
        //
        // h2                    
        //
	...
	if(!richTuple){
	  ...
	  Ngenlep = ((mc_type_&0x0F00)>>8)+(mc_type_&0x000F)-((mc_type_&0x00F0)>>4); 
	}
	...
	for(unsigned int j=0; j<IsoTrkVetoElsPt_->size(); j++){
	  ...
	  for(unsigned int iEl=0; iEl<RA4ElsVetoPt_->size(); iEl++){
	    if(getDR(IsoTrkVetoElsEta_->at(j), RA4ElsVetoEta_->at(iEl), IsoTrkVetoElsPhi_->at(j), RA4ElsVetoPhi_->at(iEl)) < isoTrkdR){
	      isoTrkdR = getDR(IsoTrkVetoElsEta_->at(j), RA4ElsVetoEta_->at(iEl), IsoTrkVetoElsPhi_->at(j), RA4ElsVetoPhi_->at(iEl));
	    }
	  }
	  for(unsigned int iMu=0; iMu<RA4MusVetoPt_->size(); iMu++){
	    if(getDR(IsoTrkVetoElsEta_->at(j), RA4MusVetoEta_->at(iMu), IsoTrkVetoElsPhi_->at(j), RA4MusVetoPhi_->at(iMu)) < isoTrkdR){
	      isoTrkdR = getDR(IsoTrkVetoElsEta_->at(j), RA4MusVetoEta_->at(iMu), IsoTrkVetoElsPhi_->at(j), RA4MusVetoPhi_->at(iMu));
	    }
	  }
	  if(isoTrkdR > 0.3) 
	...
	for(unsigned int j=0; j<IsoTrkVetoMusPt_->size(); j++){
	  ...
	  for(unsigned int iEl=0; iEl<RA4ElsVetoPt_->size(); iEl++){
	    if(getDR(IsoTrkVetoMusEta_->at(j), RA4ElsVetoEta_->at(iEl), IsoTrkVetoMusPhi_->at(j), RA4ElsVetoPhi_->at(iEl)) < isoTrkdR){
	      isoTrkdR = getDR(IsoTrkVetoMusEta_->at(j), RA4ElsVetoEta_->at(iEl), IsoTrkVetoMusPhi_->at(j), RA4ElsVetoPhi_->at(iEl));
	    }
	  }
	  for(unsigned int iMu=0; iMu<RA4MusVetoPt_->size(); iMu++){
	    if(getDR(IsoTrkVetoMusEta_->at(j), RA4MusVetoEta_->at(iMu), IsoTrkVetoMusPhi_->at(j), RA4MusVetoPhi_->at(iMu)) < isoTrkdR){
	      isoTrkdR = getDR(IsoTrkVetoMusEta_->at(j), RA4MusVetoEta_->at(iMu), IsoTrkVetoMusPhi_->at(j), RA4MusVetoPhi_->at(iMu));
	    }
	  }
	  if(isoTrkdR > 0.3) 
	...
	for(unsigned int j=0; j<IsoTrkVetoHadPt_->size(); j++){
	  ...
	  for(unsigned int iEl=0; iEl<RA4ElsVetoPt_->size(); iEl++){
	    if(getDR(IsoTrkVetoHadEta_->at(j), RA4ElsVetoEta_->at(iEl), IsoTrkVetoHadPhi_->at(j), RA4ElsVetoPhi_->at(iEl)) < isoTrkdR){
	      isoTrkdR = getDR(IsoTrkVetoHadEta_->at(j), RA4ElsVetoEta_->at(iEl), IsoTrkVetoHadPhi_->at(j), RA4ElsVetoPhi_->at(iEl));
	    }
	  }
	  for(unsigned int iMu=0; iMu<RA4MusVetoPt_->size(); iMu++){
	    if(getDR(IsoTrkVetoHadEta_->at(j), RA4MusVetoEta_->at(iMu), IsoTrkVetoHadPhi_->at(j), RA4MusVetoPhi_->at(iMu)) < isoTrkdR){
	      isoTrkdR = getDR(IsoTrkVetoHadEta_->at(j), RA4MusVetoEta_->at(iMu), IsoTrkVetoHadPhi_->at(j), RA4MusVetoPhi_->at(iMu));
	    }
	  }
	  if(isoTrkdR > 0.3) 
	    goodIsoTrkHad.push_back(j);
	}
	int nGoodIsoTrks = goodIsoTrkEls.size() + goodIsoTrkMus.size() + goodIsoTrkHad.size();
	...
	/*	for(unsigned int j=0; j<goodIsoTrkEls.size(); j++){
	  ...
	  for(unsigned int iEl=0; iEl<RA4ElsVetoPt_->size(); iEl++){
	    FillTH1FAll(h1_dR_Lep_Trk, 3, getDR(IsoTrkVetoElsEta_->at(goodIsoTrkEls.at(j)), RA4ElsVetoEta_->at(iEl), IsoTrkVetoElsPhi_->at(goodIsoTrkEls.at(j)), RA4ElsVetoPhi_->at(iEl)), EventWeight_);
	  }
	  for(unsigned int iMu=0; iMu<RA4MusVetoPt_->size(); iMu++){
	    FillTH1FAll(h1_dR_Lep_Trk, 3, getDR(IsoTrkVetoElsEta_->at(goodIsoTrkEls.at(j)), RA4MusVetoEta_->at(iMu), IsoTrkVetoElsPhi_->at(goodIsoTrkEls.at(j)), RA4MusVetoPhi_->at(iMu)), EventWeight_);
	  }
	}
	for(unsigned int j=0; j<goodIsoTrkMus.size(); j++){
	  ...
	  for(unsigned int iEl=0; iEl<RA4ElsVetoPt_->size(); iEl++){
	    FillTH1FAll(h1_dR_Lep_Trk, 3, getDR(IsoTrkVetoMusEta_->at(goodIsoTrkMus.at(j)), RA4ElsVetoEta_->at(iEl), IsoTrkVetoMusPhi_->at(goodIsoTrkMus.at(j)), RA4ElsVetoPhi_->at(iEl)), EventWeight_);
	  }
	  for(unsigned int iMu=0; iMu<RA4MusVetoPt_->size(); iMu++){
	    FillTH1FAll(h1_dR_Lep_Trk, 3, getDR(IsoTrkVetoMusEta_->at(goodIsoTrkMus.at(j)), RA4MusVetoEta_->at(iMu), IsoTrkVetoMusPhi_->at(goodIsoTrkMus.at(j)), RA4MusVetoPhi_->at(iMu)), EventWeight_);
	  }
	}
	for(unsigned int j=0; j<goodIsoTrkHad.size(); j++){
	  ...
	  for(unsigned int iEl=0; iEl<RA4ElsVetoPt_->size(); iEl++){
	    FillTH1FAll(h1_dR_Lep_Trk, 3, getDR(IsoTrkVetoHadEta_->at(goodIsoTrkHad.at(j)), RA4ElsVetoEta_->at(iEl), IsoTrkVetoHadPhi_->at(goodIsoTrkHad.at(j)), RA4ElsVetoPhi_->at(iEl)), EventWeight_);
	  }
	  for(unsigned int iMu=0; iMu<RA4MusVetoPt_->size(); iMu++){
	  ...
	  Nbcsvm_thres++;
	    ...
	    if(!richTuple){
	      ...
	      if(nmus_==1 ) dRlep = getDR( RA4MusEta_->at(0), FatjetEta_->at(ifj), 
							RA4MusPhi_->at(0), FatjetPhi_->at(ifj) );
	      if(nels_==1 ) dRlep = getDR( RA4ElsEta_->at(0), FatjetEta_->at(ifj), 
							RA4ElsPhi_->at(0), FatjetPhi_->at(ifj) );
	    }
	    ...
	    if(!richTuple){
	      ...
	      if(RA4ElsPt_->size()==1 ) dPhiMETlep=getDPhi(RA4ElsPhi_->at(0), METPhi_);
	       ...
	       if(nmus_==1 ) dPhiMETlep=getDPhi(RA4MusPhi_->at(0), METPhi_);
	       if(nels_==1 ) dPhiMETlep=getDPhi(RA4ElsPhi_->at(0), METPhi_);
	    }
            // 
	...
	int nGoodIsoTrks = 0; //FOR DEBUGGING PURPOSES ONLY
	if( !PassBaselineSelection(HT_thres, MET_, Nbcsvm_thres, Nskinny_thres, nGoodIsoTrks) ) continue;
	if( !PassSelection(Region, HT_thres, MET_, Nbcsvm_thres, Nskinny_thres, mT, MJ_thres, nGoodIsoTrks)) continue;
...
//            FillTH1FAll(h1_N4,          NFJbin, FatjetN_->at(5),      EventWeight_); 
    ...
    HistFileName = Form("~/cms26/MJ_Out/HistFiles/YieldsBook/%s/%s_%s.root", babyName, HistFileName.Data(), Region);
    ...
    for(int i=0; i<7; i++)  
    {
        ...
        h1_nGoodIsoTrks[i]->SetDirectory(0);                h1_nGoodIsoTrks[i]->Write();
