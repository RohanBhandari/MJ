...
bool PassBaselineSelection(float HT, float MET, int Ncsvm, int Nskinny, int nGoodIsoTrks=0)
{
  ...
  //  return  (HT>500 && MET>200 && Nskinny>4); //For yields book. No Nbtag cut


  return  (HT>500 && MET>200 && Nskinny>4); //DEBUGGING: JUST TO SYNC WITH RICHTUPLES
}
...
bool PassSelection(TString Region, float HT, float MET, int Nb, int Njet, float mT, float MJ, int nGoodIsoTrks)
{
  ...
  if(!richTuple){
    Nobj = (RA4MusPt_->size()+RA4ElsPt_->size()+Njet);      
  }
  else{ Nobj = (nmus_+nels_+Njet);  }
  
  int Nlep = -5;
  
  if(!richTuple){
    // For Group A
    if((RA4MusPt_->size()+RA4ElsPt_->size())==1)
      Nlep = 1;
    if((RA4MusPt_->size()+RA4ElsPt_->size())==2)
      Nlep = 2;
    //      if((RA4MusPt_->size()+RA4ElsPt_->size())==1 && (RA4MusVetoPt_->size() + RA4ElsVetoPt_->size())==0)
    //Nlep = 3;
    //      if((RA4MusPt_->size()+RA4ElsPt_->size())==1 && (RA4MusVetoPt_->size() + RA4ElsVetoPt_->size())==1)
    //	Nlep = 4;


    //For Group B
    /*      if((RA4MusPt_->size()+RA4ElsPt_->size())==1 && (RA4MusVetoPt_->size() + RA4ElsVetoPt_->size())==0 && (nGoodIsoTrks)==0)
	    Nlep = 1;
	    if((RA4MusPt_->size()+RA4ElsPt_->size())==2 && (RA4MusVetoPt_->size() + RA4ElsVetoPt_->size())==0 && (nGoodIsoTrks)==0)
	    Nlep = 2;
	    if((RA4MusPt_->size()+RA4ElsPt_->size())==1 && (RA4MusVetoPt_->size() + RA4ElsVetoPt_->size())==1)
	    Nlep = 3;
	    if((RA4MusPt_->size()+RA4ElsPt_->size())==1 && (RA4MusVetoPt_->size() + RA4ElsVetoPt_->size())==0 && (nGoodIsoTrks)==1)
	    Nlep = 4;*/
  }  
  else {
    if((nmus_+nels_)<=2) 
  ...
  int NlepMin = -1;
	...
	mTMax = 140;	mTMin = -1;
      ...
      if(i==2){ //R2
	MJMax = 99999;	MJMin = 500;
	mTMax = 140;	mTMin = -1;
      }
      if(i==3){ //R3
	MJMax = 500;	MJMin = -1;
	mTMax = 99999;	mTMin = 140;
      }
      if(i==4){ //R4
	MJMax = 99999;	MJMin = 500;
	mTMax = 99999;	mTMin = 140;
